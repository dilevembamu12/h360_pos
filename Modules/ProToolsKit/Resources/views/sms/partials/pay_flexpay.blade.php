...
<div class="width-100 text-center f-left">
    ...
    {{-- <form action="{{ route('confirm_payment', ['id' => $plan->id]) }}" method="POST"
        onsubmit="triggerMobilemoney(this)" id="flexpay_form"> --}}

    <form
        action="{{ action([\Modules\ProToolsKit\Http\Controllers\SMSController::class, 'trigger_flexpay'], [$plan->id]) }}"
        ...
        @if (!empty($plan->second_currency) && !empty($plan->second_currency_rate))
            <button type="submit" class="flexpay-button-el" style="visibility: visible;" disabled><span
                    style="display: block; min-height: 30px;"
                    onclick="currency_code='{{ $second_currency->code }}'">Mobile Money
                    ({{ (float) $plan->second_currency_rate * (float) $total_payable_formatted }}{{ $second_currency->code }})</span></button>
...
@section('mycustom_js')
    <script>
        ...
        $("#flexpay_form").submit(function(e) {
            ...
            Swal.fire({
                ...
                inputValidator: (value) => {
                    ...
                    $.ajax({
                        url: '{{ action([\Modules\ProToolsKit\Http\Controllers\SMSController::class, 'trigger_flexpay'], [$plan->id]) }}?currency_code=' +
                            currency_code,
                        type: "POST",
                        data: $(this).serialize(),
        ...
        function checkMobileMoneyWebhook(ordernumber) {
            ...
            //$(this).find('[name="phone"]').val(value);
            $.ajax({
                url: "{{ action([\Modules\ProToolsKit\Http\Controllers\SMSController::class, 'confirm_flexpayBuycredit']) }}",
                type: "POST",
                data: {
                                ...
                                Swal.fire('Reussi!', response.msg, 'success').then(
                                    function() {
                                        window.location.href =
                                            "{{ action([\Modules\ProToolsKit\Http\Controllers\SMSController::class, 'index']) }}";
                                    });
                                return;
