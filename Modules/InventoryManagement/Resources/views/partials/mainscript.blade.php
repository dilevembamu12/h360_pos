<script>
    $(document).ready(function () {
        ...
        function parseProductOnlyNotExist(result) {
            var htmlContent = result; // a response via AJAX containing HTML
            var e = document.createElement('table');
            e.setAttribute('style', 'display: none;');
            e.setAttribute('id', 'temp_table_get_purchase_entry_row');
            e.innerHTML = htmlContent;
            document.body.appendChild(e);
            var htmlConvertedIntoDom = e.lastChild
                .childNodes; // the HTML converted into a DOM element :), now let's remove the

            var resultOnlyNotExist = "";
            $('#temp_table_get_purchase_entry_row > tbody  > tr').each(function (index, tr) {
                //console.log(index);
                //console.log(tr);
                //alert($(tr).attr('id'));
                if (!$('#purchase_entry_table').find('#' + $(tr).attr('id')).length) { //n'existe pas
                    resultOnlyNotExist += tr.outerHTML;
                }
            ...
            $('#purchase_entry_table > tbody:last-child').append(resultOnlyNotExist);
        }
        /****************************************************************/
        /************custom_code 06062024-loadallproduct_in_once ********/
        $("#loadProducts").on("click", function () {
            //alert(notExistsProducts);
            /*** obtenir l'ID de l'inventaire ***/
            ...
            /************************************************/
            var data = {
                product_id: 0, //si cest empty cad 0 donc on fait la recherche des produits pas encore listé dans l'inventaire
                variation_id: 0,
                ...
                success: function (result) {
                    if (result) {
                        try {
                            ...
                            /************custom_code 06062024-loadallproduct_in_once ********/
                            parseProductOnlyNotExist(result);
                            /****************************************************************/
                        }
                    }
                },
        ...
        /************ PERSONNALISE CODE 18022025 - FILL AUTO STOCK ******/
        $('#autoFillStock').on('click', function () {
            var table = $('#purchase_entry_table');
            var products = $(table).find('input[type="text"]');
            //alert(products.length);
            $.each(products, function (p_key, p) {

                //alert($(p).attr('id').split("_")[1]);
                input_split_id = $(p).attr('id').split("_")[1];
                qty = $(table).find(`#productQuantity_${input_split_id}`).html();
                qty = (Number(qty) == 0) ? "00" : qty;
                $(p).val(qty);
                updateInventoryAmount(this, input_split_id);
            });
        })
        /****************************************************************/
    ...
    $("#saveProducts").on("click", function (e) {
        ...
        $("#purchase_entry_table tr:not([data-skip])").each(function () {
            ...
            if (this.id > 0) {
                ...
                //si il met 00 on considere que le produit et chequé mais le stock est vide
                if (amountAfterInventory != "00") {
    ...
    /************ PERSONNALISE CODE 18022025 - MANAGE SAVING PRODUCT UNTIL FINISH ******/
    //var on_saveProducts = 0;
    /****************************************************************/
    function sendAjaxCall(obj) {
        ...
        Swal.fire({
            ...
            title: 'Mise à jour des stocks de l\'inventaire**',
            html: "Ceci peut prendre du temps veuillez patienter**" +''
                +'<br><img src="{{ asset('img/ui/inventory.gif') }}" width="150" />'
                ,
            timer: 900000,
        ...
        $.ajax({
            ...
            data: {
                //info: obj
                info: JSON.stringify(obj) //pour eviter warning-input-variables-exceeded-1000
            },
            success: function (result) {


                if (result.status == 200) {
                    Swal.fire('Reussi!', "Opération reussie**", 'success').then(
                        function () {
                    ...
                    Swal.fire(
                        'Oops',
                        'Une erreur est subvenue , veuillez réessayer plus tard**',
                        'error'
                    );
                }
            },
