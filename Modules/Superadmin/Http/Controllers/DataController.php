...
//pour reperer si le module ProToolKit est activé
...
class DataController extends Controller
{
    ...
    public function modifyAdminMenu()
    {
        ...
        /****custom_code 17062024 */
        //pour reperer si le module ProToolKit est activé
        $is_ptk = $moduleUtil->isModuleInstalled('ProToolsKit');
        /********************************** */
        ...
        if (auth()->user()->can('superadmin')) {
            ...
            */
            ...
            $menuparent->dropdown(
                ...
                function ($sub) use($is_ptk) {
                    $sub->url(action([\Modules\Superadmin\Http\Controllers\SuperadminController::class, 'index']), __('superadmin::lang.superadmin'), ['icon' => 'fa fas fa-users-cog', 'active' => request()->segment(1) == 'superadmin'])->order(0);

                    if($is_ptk ){
                        $sub->url(
                        action('\Modules\Superadmin\Http\Controllers\SMSController@index'),
                        '**SMS/WB API',
                ...
                ['icon' => 'fa fas fa-users-cog', 'active' => request()->segment(1) == 'superadmin']
            )->order(0);
            /************************************************** */
        }
