$(document).ready(function () {
    ...
    $('select#customer_id').select2({
        ajax: {
            ...
            url: '/serverfaker/fakerillusion.json',
            dataType: 'json',
            delay: 250,
            ...
            success: function (data) {
                serverfaker_search_contacts("k").then(function (result) {
                    v = 333;
                    /*
                    return {
                        results: result,
            ...
            processResults: function (data, params) {
                contact_param_q = params.term;
                console.log('results: data,', doc_contacts);
                return {
                    results: doc_contacts,
                };
                return {
                    results: doc_contacts,
                };
                ...
                serverfaker_search_contacts(params.term).then(function (result) {
                    console.log(result);
                    return {
                        results: result,
                    };
        ...
        templateResult: function (data) {
            //alert(contact_param_q);
            console.log('azerty', data);
            if(data.loading && data.loading==true ){
                return;
            }
            if ((data.supplier_business_name  && (data.supplier_business_name.toUpperCase()).includes(contact_param_q.toUpperCase())) 
            || (data.text  && (data.text.toUpperCase()).includes(contact_param_q.toUpperCase()))
            || (data.mobile  && (data.mobile.toUpperCase()).includes(contact_param_q.toUpperCase()))) {
                /*
            console.log(data);
            //alert(data);
            return 1;
        ...
        submitHandler: function (form) {
            ...
            $('.pos_unit_price_inc_tax').each(function () {
                if (!$(this).is(":visible") && $(this).data('rule-min-value')) {
                    ...
                    if (val < $(this).data('rule-min-value')) {//personnalize custom code 13022024-MINIMUM_SELL_BLOCK001
                        is_msp_valid = false;
            ...
            if (cnf) {
                ...
                var nw_sell={"_id": 1, "id":1 ,"data":$(form).serialize(),'date':__current_datetime()}
                serverfaker_put_sell(form).then(function (result) {
...
function get_product_suggestion_list(category_id, brand_id, location_id, url = null, is_enabled_stock = null, repair_model_id = null) {
    ...
    if (url == null) {
        ...
        url = '/serverfaker/get-product-suggestion.htmljjj';
    }
    //alert(url);
    ...
    serverfaker_get_product_suggestion(location_id).then(function (result) {
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = 1) {
    //alert("variation_id="+variation_id);
    ...
    //alert("item_addtn_method="+item_addtn_method);
    ...
    if (add_via_ajax) {
        ...
        */
        ...
        //alert('/sells/pos/get_product_row/' + variation_id + '/' + location_id);
        url = '/serverfaker/get_product_row.js';
        //var result=pouchdb_get_product_row(variation_id,location_id);

        serverfaker_get_product_row(variation_id, location_id, product_row).then(function (result) {
...
function pos_print(receipt) {
    ...
    //alert("receipt.print_type ==>"+receipt.print_type );
    //alert("receipt.html_content ==>"+receipt.html_content );
    ...
    if (receipt.print_type == 'printer') {
        ...
        $('#pos_html').html(receipt.html_content);
        __currency_convert_recursively($('#pos_html'));
        //__print_receipt('pos_html');
...
/*
setTimeout(function () {
    //$('div[data-variation_id="44"]').click();
    //$('div[data-variation_id="45"]').click();

    $('div[data-variation_id="3517"]').click();
    $('div[data-variation_id="3518"]').click();
    
    
    setTimeout(function () {
        $('button[data-pay_method="cash"]').click();
    }, 200);
}, 1200);
...
function get_contact_due(id) {
    //alert(id);
    doc_contacts.forEach(function (contact) {
        
        if (contact.id == id) {
            if (contact.due != '') {
                $('.contact_due_text').find('span').text(contact.due);
