$(document).ready(function () {
    ...
    product_sell_report = $('table#product_sell_report_table').DataTable({
        ...
        columns: [
            ...
            { data: 'unit_price_diff', name: 'unit_price_diff', searchable: false },
            /*********end custom */
        ],
        createdRow: function (row, data, index) {
            /*******  personnalize custom code 26042024-MANAGE_CUSTOM_PRODUCT_PRICE-001  */
            color="black";
            amount=(__currency_symbol_placement=='before')? $('td', row).eq(12).html().split(" ")[1] : $('td', row).eq(12).html().split(" ")[0];
            currency_code=(__currency_symbol_placement=='before')? $('td', row).eq(12).html().split(" ")[0] : $('td', row).eq(12).html().split(" ")[1];
            sign="";
            //alert(amount);
            if(Number(amount) > 0) {
                color="color:green;font-weight: bold;";sign="+";
            }else if(Number(amount) < 0) {
                color='color:red;font-weight: bold;';
            }

            if(__currency_symbol_placement=='before') {
                str='<span class="row_unit_price_diff" style="'+color+'" data-orig-value="'+$('td', row).eq(12).html()+'">'+currency_code+' '+sign+amount+'</span>';
            }else  {
                str='<span class="row_unit_price_diff" style="'+color+'" data-orig-value="'+$('td', row).eq(12).html()+'">'+sign+amount+' '+currency_code+'</span>';
            }
            //var str='<span class="row_unit_price_diff" data-orig-value="16.00000000">$ 16.00</span>';
            $('td', row).eq(12).html(str);
        ...
        fnDrawCallback: function (oSettings) {
            ...
            $('#footer_unit_price_diff').text(
                sum_table_col($('#product_sell_report_table'), 'row_unit_price_diff')
            );
