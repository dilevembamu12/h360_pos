...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction(
        ...
        $transaction = Transaction::create([
            ...
            //{{--  personnalize custom code 23032024-MULTIDEVISE030 -- 24032024}}
            //==>on fait passer $location_details en argument pour recuper le taux de coversion
            'second_currency' => !empty($location_details) ? $location_details->second_currency : null,
            'second_currency_rate' => !empty($location_details) ? $location_details->second_currency_rate : null,
            /********** end personnalize******** */
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                //gerer les prix de produits modifiés afin de nous pemettre de calculer la difference des benefices excedentaire
                ...
                //dd($product['unit_price_inc_tax']*$product['quantity']-$variation_details['sell_price_inc_tax']*$product['quantity']);
                ...
                $line = [
                    ...
                    /*******  personnalize custom code 26042024-MANAGE_CUSTOM_PRODUCT_PRICE-001  */
                    //'unit_price_before_customupdate' =>$variation_details['sell_price_inc_tax']+$line_discount_amount,//on tient en compte les remises sur le produit()  
                    //'unit_price_before_customupdate' => $product['unit_price_inc_tax'] - $variation_details['sell_price_inc_tax']+$line_discount_amount,
                    'unit_price_before_customupdate' =>$variation_details['sell_price_inc_tax'],
                    /*********end custom */
     ...
     * @return bool
     */
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true, $second_currency_id = null, $rate = null)
    {
        $payments_formatted = [];
        $edit_ids = [0];
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    //{{--  personnalize custom code 06022024-MULTIDEVISE030 -- 09032024}}
                    //à ne pas utiisercar les motant viennent deja converti au taux par defauy du systeme
                    /*
                    if (!empty($second_currency_id) && !empty($rate)) {
                        $payment_amount /= $rate;
                    }
                    */
                    //***************************************************************** */
                    ...
                    $payment_data = [
                        ...
                        //{{--  personnalize custom code 06022024-MULTIDEVISE030 -- 09032024}}
                        //suivre le seconde devise pour permetrre de l'enregistrer dans la table transactions
                        'second_currency_rate' => $rate,
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if (!empty($il->sub_heading_line5)) {
            $il->sub_heading_line5 = "1" . $business_details['currency_code'] . "=" . $this->num_f($transaction->second_currency_rate) . $mycurrency->code;
        }
        //----------------------- END PERSONNALIZE CUSTOM CODE-----------------------------------//////
        $output = [
        ...
        if (!empty($location_details->second_currency)) {
            $mycurrency = Currency::find($location_details->second_currency);
            $output['currency_symbol'] = $mycurrency->symbol;
        }
        //----------------------- END PERSONNALIZE CUSTOM CODE-----------------------------------//////
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            foreach ($lines as $key => $value) {
                $lines[$key]->unit_price_before_discount *= $transaction->second_currency_rate;
                $lines[$key]->unit_price *= $transaction->second_currency_rate;

                $lines[$key]->line_discount_amount = ($lines[$key]->line_discount_type == 'fixed') ? $lines[$key]->line_discount_amount * $transaction->second_currency_rate : $lines[$key]->line_discount_amount;


                $lines[$key]->unit_price_inc_tax *= $transaction->second_currency_rate;
                $lines[$key]->item_tax *= $transaction->second_currency_rate;
            }
            //----------------------- END PERSONNALIZE CUSTOM CODE-----------------------------------//////
            ...
            $output['subtotal_exc_tax'] = $this->num_f(
                $subtotal_exc_tax,
                true,
                $business_details /* personnalize custom code 06022024-MULTIDEVISE030 */,
                false,
                $output['currency_symbol']
                /** */
            );
            $output['total_line_discount'] = !empty($total_line_discount) ? $this->num_f(
                $total_line_discount,
                true,
                $business_details/* personnalize custom code 06022024-MULTIDEVISE030 */,
                false,
        ...
        $output['subtotal'] = ($transaction->total_before_tax != 0) ? $this->num_f(
        ...
        $output['round_off'] = $this->num_f(
            $transaction->round_off_amount,
            $show_currency,
            $business_details/* personnalize custom code 06022024-MULTIDEVISE030 */,
            false,
            $output['currency_symbol']
        ...
        $output['total_exempt'] = $this->num_f(
            $total_exempt,
            $show_currency,
            $business_details/* personnalize custom code 06022024-MULTIDEVISE030 */,
            false,
        ...
        $output['taxed_subtotal'] = $this->num_f(
            $taxed_subtotal,
            $show_currency,
            $business_details/* personnalize custom code 06022024-MULTIDEVISE030 */,
            false,
        ...
        $output['discount_label'] .= ($transaction->discount_type == 'percentage') ? ' <small>(' . $this->num_f(
        ...
        $output['discount'] = ($discount != 0) ? $this->num_f(
            $discount,
            $show_currency,
            $business_details/* personnalize custom code 06022024-MULTIDEVISE030 */,
            false,
        ...
        if ($business_details->enable_rp == 1 && !empty($transaction->rp_redeemed)) {
            ...
            $output['reward_point_amount'] = $this->num_f(
                $transaction->rp_redeemed_amount,
                $show_currency,
                $business_details/* personnalize custom code 06022024-MULTIDEVISE030 */,
                false,
                $output['currency_symbol']
        ...
        if (!empty($output['taxes'])) {
            ...
            $output['taxes'][trans('lang_v1.total_tax')] = $this->num_f(
                $total_tax,
                $show_currency,
                $business_details/* personnalize custom code 06022024-MULTIDEVISE030 */,
                false,
        ...
        $output['tax_label'] .= ':';
        $output['tax'] = ($transaction->tax_amount != 0) ? $this->num_f(
        ...
        //Shipping charges
        $output['shipping_charges'] = ($transaction->shipping_charges != 0) ? $this->num_f(
        ...
        $output['packing_charge'] = ($transaction->packing_charge != 0) ? $this->num_f(
            $transaction->packing_charge,
            $show_currency,
            $business_details/* personnalize custom code 06022024-MULTIDEVISE030 */,
            false,
            $output['currency_symbol']
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            $output['total_paid'] = ($paid_amount == 0) ? 0 : $this->num_f(
                $paid_amount,
                $show_currency,
                $business_details/* personnalize custom code 06022024-MULTIDEVISE030 */,
                false,
            ...
            $output['total_due'] = ($due == 0) ? 0 : $this->num_f(
                $due,
                $show_currency,
                $business_details/* personnalize custom code 06022024-MULTIDEVISE030 */,
                false,
        ...
        if (!empty($transaction->additional_expense_value_1) && !empty($transaction->additional_expense_key_1)) {
            $output['additional_expenses'][$transaction->additional_expense_key_1] = $this->num_f(
        ...
        if (!empty($transaction->additional_expense_value_2) && !empty($transaction->additional_expense_key_2)) {
            $output['additional_expenses'][$transaction->additional_expense_key_2] = $this->num_f(
        ...
        if (!empty($transaction->additional_expense_value_3) && !empty($transaction->additional_expense_key_3)) {
            $output['additional_expenses'][$transaction->additional_expense_key_3] = $this->num_f(
        ...
        if (!empty($transaction->additional_expense_value_4) && !empty($transaction->additional_expense_key_4)) {
            $output['additional_expenses'][$transaction->additional_expense_key_4] = $this->num_f(
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            if ($zatca_qr) {
                ...
                if (in_array('total_tax', $qr_code_fields)) {
                    $total_order_tax = $transaction->tax_amount + $total_line_taxes;
                    $total_order_tax_formatted = $this->num_f(
                        $total_order_tax,
                        $show_currency,
                        $business_details/* personnalize custom code 06022024-MULTIDEVISE030 */,
                        false,
    ...
    public function getExpenseReport(
        ...
        $query = Transaction::leftjoin('expense_categories AS ec', 'transactions.expense_category_id', '=', 'ec.id')
            ...
            /*******  personnalize custom code 09092024-PRENDRE EN CHARGE AUSSI LES SALAIRE DES COMMERCIAUX ==>"payroll" */
            ->whereIn('type', ['expense', 'expense_refund','payroll']);
            /*********end custom */
    ...
    public function purchaseCurrencyDetails(
        ...
        /********  personnalize custom code 06022024-MULTIDEVISE030 ****/
        //si le currency a été passé en argument , donc je overide le currency par defaut
    ...
    public function getTransactionTotals(
        ...
        */
        ...
        $start_date="2024-09-01 23:59:00";
        */
        if (!empty($start_date) && !empty($end_date)) {
            $query->whereDate('transactions.transaction_date', '>=', $start_date)
                ->whereDate('transactions.transaction_date', '<=', $end_date);
        ...
        if (in_array('payroll', $transaction_types)) {
            //dd($transaction_totals->total_payroll);
