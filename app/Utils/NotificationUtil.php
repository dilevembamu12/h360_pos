...
class NotificationUtil extends Util
{
    ...
    public function configureEmail($notificationInfo = [], $check_superadmin = true)
    {
        ...
        if (! empty($is_superadmin_settings_allowed) && ! empty($email_settings['use_superadmin_settings']) && $check_superadmin) {
            ...
            //{{--  personnalize custom code 19052024-MAIL FIX ERROR OF NO DATA -- 19052024}}
        ...
        /*
        $a=Config::get('mail.default');
        
        dd($email_settings);
        dd(env('MAIL_HOST'));
        dd(Config::get('mail.smtp.transport'));
        dd($is_superadmin_settings_allowed."===".$email_settings['use_superadmin_settings']."====".$check_superadmin);
        */
