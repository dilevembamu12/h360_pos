...
/*  personnalize custom code 22032025-H360GPT-AUTOSTOCK-TRAIN -- 22032025 */
...
class ProductUtil extends Util
{
    ...
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0)
    {
        ...
        /***** CUSTOM: pour actualiser les stock en temps réel *********** */
        /*
        $url=action([\App\Http\Controllers\ProductController::class, 'productStockHistory'], [$product_id]).
        '?location_id='.$location_id.'&variation_id='.$variation_id;
    ...
    public function updateProductFromPurchase($variation_data)
    {
        ...
        /*  personnalize custom code 22032025-H360GPT-AUTOSTOCK-TRAIN -- 22032025 */
        //event(new ProductsCreatedOrModified($variation_details->product, 'updated'));
        /********** end personnalize */
    }
     ...
     * @return array
     */
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null, $second_currency_id = null, $rate = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            if (!empty($second_currency_id) && !empty($rate)) {
                $purchase_line->pp_without_discount /= $rate;
                $purchase_line->discount_percent /= $rate;
                $purchase_line->purchase_price /= $rate;
                $purchase_line->purchase_price_inc_tax /= $rate;
                $purchase_line->item_tax /= $rate;



                $purchase_line->second_currency = $second_currency_id;
                $purchase_line->second_currency_rate = $rate;
            ...
            if ($enable_product_editing == 1 && $transaction->type == 'purchase') {
                ...
                //{{--  personnalize custom code 06022024-MULTIDEVISE030 -- 09032024}}
                //il faut traquer jusqu'ici car la mise à jour modifie le prix de vente du produit ce qu'on a geré en convertissant les données
                //-> à la devise par defaut afin de maintenir le prix de vente du produit exacte comme ca a été crée dès l'origine
                if (!empty($second_currency_id) && !empty($rate)) {

                    //$variation_data['purchase_price'] //pas lui car il avait deja été converti

                    $variation_data["sell_price_inc_tax"] /= $rate;
                    $variation_data["pp_without_discount"] /= $rate;
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        //yhu$query->addSelect(DB::raw('variations.sell_price_inc_tax * 2700 as selling_price'));
     ...
     public function getProductStockDetails($business_id, $filters, $for, $user_id = null /***personnilize force l'id utilisateur */)
    {
        ...
        $products = $query
            ->leftjoin('brands as b', 'p.brand_id', '=', 'b.id') //custom
            ->leftjoin('units as u', 'p.unit_id', '=', 'u.id') //custom
            ->select(
            ...
            ->selectSub(function ($query) use ($filters) {
                /**********CUSTOM ************/
                $sub_units = [];
                //$h=json_encode(["nom"=>"tete"]);
                if (!empty($filters['product_id'])) {
                    $product = Product::where('id', $filters['product_id'])
                        ->with(['unit', 'second_unit'])
                        ->first();
                    $business_id = request()->session()->get('user.business_id');
                    $sub_units = $this->getSubUnits($business_id, $product->unit->id, false, $filters['product_id']);

                    foreach ($sub_units as $key => $sub_unit) {
                        # code...
                    }
                }
                $a = base64_encode(json_encode($sub_units));
                $query->selectRaw('"' . $a . '"');
                /*********/
            }, 'subunits')
            /******************* */
    ...
    public function getVariationStockHistory($business_id, $variation_id, $location_id)
    {
        ...
        foreach ($stock_history as $stock_line) {
            $temp_array = [
                ...
                'additional_notes' => $stock_line->additional_notes, //custom
            ];
