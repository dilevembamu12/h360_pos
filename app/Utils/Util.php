...
class Util
{
    ...
    /******* personnalize custom code 24072024 */
    private function sendSmsViaProToolKit($data)
    {
        $numbers = explode(',', trim($data['mobile_number']));
        //$nexmo = app('Modules\ProToolsKit\Http\Controllers\SMSController::class');
        ...
        $_request->replace([
            '_token' => csrf_token(),
            'auto_sms'=>true, // si la requete vient d'un auto sms
        ]);
        
        foreach ($numbers as $number) {
            $_request->request->add(['manual_to' => $number]);
            $_request->request->add(['sms_body' => $data['sms_body']]);

            $result = \App::makeWith(\Modules\ProToolsKit\Http\Controllers\SMSController::class)->manualSendSms($_request);
            //dd($result);
        }
    }
    /******* end personnalize custom code 24072024 */
    ...
    public function sendSms($data)
    {
        ...
        /******* personnalize custom code 24072024 */
        //je verifie qu'il ne s'agit pas de business_id de l'entreprise h360
        //http://localhost:8001/protoolskit/manualsendsms
        $business_id = session()->has('business') ? session('business.id') : 0;
        if($business_id!=env('ADMINISTRATOR_BUSINESS')){
            return $this->sendSmsViaProToolKit($data);
        }
        /******* end personnalize custom code 24072024 */
    ...
    public function replaceTags($business_id, $data, $transaction, $contact = null)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (strpos($value, '{business_logo}') !== false) {
                $logo_name =str_replace('+','%20',urlencode($business->logo));
