...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getRegister()
    {
        ...
        /**   personnalize custom code 06012025-GESTION AFFILIATION ***/
        //RECUPERER TOUS LES AFFILIES
        $affilates_parent = User::where('business_id', env('ADMINISTRATOR_BUSINESS'))
        ...
        $affiliates = [];
        foreach ($affilates_parent as $key => $parent) {
            $affiliates[$parent->id] = $parent->full_name . ' (ID:' . $parent->id.")";
        }

        $_countries=Country::select(['sortname','name'])->get();
        $countries=[];
        foreach ($_countries as $key => $_country) {
            $countries[$_country->sortname]=$_country->name;
        }
        //dd($countries);
        /******************************************************* */
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            ...
            /**   personnalize custom code 06012025-GESTION AFFILIATION ***/
            //je verifie si l'affiliation existe sinon on utilise le compte d'affiliation par defaut
            //$business_id
            //$request->affiliate_id
            //dd(11);
            $affiliate_id=empty($request->affiliate_id) ? env('H360POS_BUSINESS_DEFAULT_AFFILIATE_ID') : $request->affiliate_id;
            
            Artisan::call('pos:businessOwnerContact --affiliate_id='.$affiliate_id);
            //Artisan::call('pos:businessOwnerContact --business_id='.$business->id.' --affiliate_id='.$affiliate_id);
            //dd('pos:businessOwnerContact --business_id='.$business->id.' aaaa --affiliate_id='.$affiliate_id);
            /****************************************************************************************** */
            ...
            /*** personnalize 01042025 auto sign after create business ***/
            Auth::login($user);
            return redirect('subscription')->with('status', $output);
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            //obtiens les parametre de toutes les devises passées en argument selon la formule currency_{id}
            $currencies = $this->businessUtil->allCurrencies();
            $_currencies = [];
            foreach ($currencies as $key => $value) {
                //je ne recupere que ceux qui sont => currency_is_enabled_1=1
                if($request->input('currency_is_enabled_'.$key)!=1) continue;
                $_currencies[]='currency_'.$key;
            }
            /******************************************************* */
            ...
            $business_details = $request->only(array_merge(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
                ...
                'rp_expiry_type', 'custom_labels', 'weighing_scale_setting',
                'code_label_1', 'code_1', 'code_label_2', 'code_2', 'currency_precision', 'quantity_precision'
                /*personnalize custom code 18092024-001 gestion de HEURE unité de salire*/ ,'hour_precision'/**/
            ],$_currencies));
            /*********************************************************/
            //dd($business_details);
            /*personnalize custom code 18092024-001 gestion de HEURE unité de salire*/
            //on converti tout en nombre au cas contraire on met 0 pour signifier que l'entreprise n'a pas de conversion automatique
            $business_details['hour_precision'] = ! empty($business_details['hour_precision']) ? $this->businessUtil->num_uf($business_details['hour_precision']) : 60;
            /**/
            ...
            /**   personnalize custom code 06022024-MULTIDEVISE002 ***/
            //je veifie que la donnée entrée est conforme aux conditions
            $__currencies=[];
            foreach ($_currencies as $key => $value) {
                //je ne recupere que ceux qui sont => currency_is_enabled_1=1
                //if($request->input('currency_is_enabled_'.$key)!=1) continue;
                $__currencies[$value]= filter_var($business_details[$value], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);//le convertir en nombre au lieu que ca soi en format monnai qui est une caine  
            }
            $business_details['second_currency_settings'] = ! empty($__currencies) ? $__currencies : [];
            //dd($business_details);
            //dd($__currencies);
    ...
    /**   personnalize custom code 14122024-RESET_REWARD ***/
    /**
     * Updates business settings
     *
     * @param  \Illuminate\Http\Request  $request
    ...
    public function resetRewards(Request $request)
    {
        ...
        try {
            //Artisan::call('pos:businessOwnerContact --business_id=63 --affiliate_id=98');
            Artisan::call('pos:updateRewardPoints --reset');
            ...
            $output = [
                'success' => 1,
                'msg' => "**Cumul d'achat reinitialisé avec success",
            ];
        } catch (\Exception $e) {
            \Log::emergency('File:' . $e->getFile() . 'Line:' . $e->getLine() . 'Message:' . $e->getMessage());
