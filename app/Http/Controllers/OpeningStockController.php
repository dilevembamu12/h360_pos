                    ...
                    if (array_key_exists($location_id, $locations)) {
                        ...
                        foreach ($value as $vid => $purchase_lines_data) {
                            ...
                            foreach ($purchase_lines_data as $k => $pl) {
                                ...
                                //on ne modifie pas les transactions venant de l'inventaire
                                if (isset($pl['purchase_line_id'])) {
                                    ...
                                    $transaction = Transaction::where('type', 'opening_stock')
                                        ...
                                        ->find($purchase_line->transaction_id);
                                        //dd($purchase_line);
                                        //dd($pl);
                                        
                                    
                                    $from_inventory_attribute = '';
                                    if (str_contains($transaction->additional_notes, 'INV-') &&
                                        str_contains($transaction->additional_notes, ': INVENTORY BY') && 
                                        str_contains($transaction->additional_notes, '[ID:') ) {
                                        //continue;
                                            /*
                                        
                                            //$pl["quantity"]=$purchase_line->quantity_used;
                                            $pl["quantity"]=$purchase_line->quantity;
                                            $pl["purchase_price"] = $purchase_line->purchase_price;
                                            $pl["transaction_date"] =\Carbon::parse($transaction->transaction_date)->format(session('business.date_format') . ' H:i');
                                            $pl["purchase_line_note"] =$transaction->additional_notes;
                                            */
                                    ...
                                    //dd($transaction->additional_notes);
                        ...
                        if (! empty($new_purchase_lines)) {
                            ...
                            foreach ($new_purchase_lines as $key => $new_purchase_line) {
                                ...
                                //Adjust stock over selling if found
                                $a=$this->productUtil->adjustStockOverSelling($transaction);
