...
class AccountController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            $is_closed = request()->input('account_status') == 'closed' ? 1 : 0; //BEFORE PERSONNALIZE 17022025
            /**   personnalize custom code 17022025-GESTION DES COMPTE DE PAIEMENT SUPRIMES ***/
            //cette mise à jour generer une erreur raison pourlaquelle on enleveca pour le moment
            /*
            $is_closed =  0;//COMPTE OUVERTS
            $aaa=111;
            if(request()->input('account_status') == 'closed'){
                $is_closed =  1;
                $accounts->where('is_closed', $is_closed);
                //->whereNull('AT.deleted_at');
            }elseif (request()->input('account_status') == 'deleted') {
                $is_closed =  2;
                $accounts->where('is_closed', 5);//POUR NE RIEN AFFICHER
                //$accounts->whereNotNull('AT.deleted_at');
            }else{
                $is_closed =  0;
                $accounts->where('is_closed', $is_closed);
                //->whereNull('AT.deleted_at');
            ...
            */
            ...
            /********* BEFORE PERSONNALIZE 17022025 ********/
            
            $accounts->where('is_closed', $is_closed)
                ->whereNull('AT.deleted_at')
                ->groupBy('accounts.id');
            /********************************************** */
            //$accounts->groupBy('accounts.id'); //mise à jour ca generer une erreur dans la copmptabilite
            /*****END ******/

            /**   personnalize custom code 17022025-GESTION DES COMPTE DE PAIEMENT SUPRIMES ***/
            $datatable= DataTables::of($accounts)
                ->addColumn(
                    'action',
                ...
                ;
                /**** ca n'a pas marché *
                if ($is_closed==2) {//si on verifie seulement les comptés qui ont été supprimé ou rendu invisible
            ...
            return $datatable->removeColumn('id')
    ...
    public function index2()
    {
        ...
        return view('manuel_report_cash.index')
            ->with(compact('not_linked_payments', 'account_types'));
    }
    ...
    public function getFundTransfer($id)
    {
        ...
        if (request()->ajax()) {
            ...
            /* custom code 001-1-2 */
            //on affiche que les comptes dont il a la permission
            foreach ($to_accounts as $key => $to_account) {
                if (!auth()->user()->can('account.tba.' . $key)) {
                    unset($to_accounts[$key]);
                }
            }
            /********************** */
    ...
    public function postFundTransfer(Request $request)
    {
        ...
        try {
            ...
            /* custom code 001-1-5 */
            //JE VERIFIE QUE LA TRANSACTION NE SE FAIT PAS DANS LE MEME COMPTE
            if ($from==$to) {
                $output = [
                    'success' => false,
                    'msg' => "L'opération ne peut aboutir avec ce compte destinateur",
                ];
            }
            //je verifie si il a les permission
            if (!auth()->user()->can('account.tba.' . $from)) {
                $output = [
                    'success' => false,
                    'msg' => "Vous n'avez pas l'autorisation réquise pour transferer le fond de ce compte",
                ];
            }
            if (!auth()->user()->can('account.tba.' . $to)) {
                $output = [
                    'success' => false,
                    'msg' => "Vous n'avez pas l'autorisation réquise pour transferer le fond à ce compte",
                ];
            }

            //obligation de publier une piece jointe
            if (!$request->hasFile('document')) {
    
                //If multiple files present
                $output = [
                    'success' => false,
                    'msg' => "Nous ne pouvons pas executer cette opération sans aucune piece justificative, document, ou bordereau",
                ];
            }
            if($output ){return redirect()->action([\App\Http\Controllers\AccountController::class, 'index'])->with('status', $output);}
            /********************** */
    ...
    public function activate($id)
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                $account->is_closed = 0;
                /**   personnalize custom code 17022025-GESTION DES COMPTE DE PAIEMENT SUPRIMES ***/
                //par moment le champ AT.deleted_at est devient parfois non null 
                // et cela rend les compte invisible comme si il a été supprimé quelque part
                // raison pour laquelle quand on active un compte je supprime la date de suppression automatiquement
                //dd($account->delete_at);
                //$account->delete = null; //ca generere une erreur dans les comptabilité on enleve ces mises à jour
                /***************************************************************************** */
                $account->save();
