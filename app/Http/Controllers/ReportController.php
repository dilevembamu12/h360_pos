...
class ReportController extends Controller
{
     ...
     * @return \Illuminate\Http\Response
     */
    public function getManuelreport(Request $request)
    {
        ...
        /***********CUSTOM ***********************/
        /**REASON: ACTIVER LE CANOPY */
        /*
        $location=null;
        $locations_canopy_ids=[];
        if($filters['location_id']){
        $location=BusinessLocation::find($filters['location_id']);
        $locations_canopy_ids=explode(',',$location->custom_field1);
        }
        dd($location);
        ...
        $report_products = [];
        $report_expenses = [];
        //custom_field1=>ID site enfants  separé par virgule
        //$locations_canopy_ids = explode(',',BusinessLocation::where('custom_field1', $row->product_id)->get()[0]);
        dd($row);
        return $locations_canopy;
        */
        /********ENDCUSTOM ********************/
        ...
        if ($for == 'view_product' && !empty(request()->input('product_id'))) {
            ...
            $show_by_box = $filters['show_by_box'];

            return view('product.partials.product_stock_details')->with(compact('product_stock_details', 'show_by_box'));
        }

        if (isset($filters['show_by_box']) && $filters['show_by_box'] == 'true') {
            $datatable = Datatables::of($products)
                ->editColumn('stock', function ($row) {
                    $product = Product::where('id', $row->product_id)
                        ...
                        ->first();
                    $business_id = request()->session()->get('user.business_id');
                    $sub_units = $this->productUtil->getSubUnits($business_id, $product->unit->id, false, $row->product_id);
                    ...
                    if ($row->enable_stock) {
                        ...
                        return '<span class="current_stock" data-orig-value="' . (float) ($stock / array_reverse($sub_units)[0]['multiplier']) . '" data-unit="' . explode(' ', array_reverse($sub_units)[0]['name'])[0] . '"> ' . $this->transactionUtil->num_f(($stock / array_reverse($sub_units)[0]['multiplier']), false, null, true) . '</span>' . ' ' . explode(' ', array_reverse($sub_units)[0]['name'])[0];
                    } else {
                        return '--';
                    }
                })
                ...
                ->editColumn('total_sold', function ($row) {
                    ...
                    return '<span data-is_quantity="true" class="total_sold" data-orig-value="' . ($total_sold / array_reverse($sub_units)[0]['multiplier']) . '" data-unit="' . explode(' ', array_reverse($sub_units)[0]['name'])[0] . '" >' . $this->transactionUtil->num_f(($total_sold / array_reverse($sub_units)[0]['multiplier']), false, null, true) . '</span> ' . explode(' ', array_reverse($sub_units)[0]['name'])[0];
                })
                ->editColumn('total_transfered', function ($row) {
                    ...
                    return '<span class="total_transfered" data-orig-value="' . ($total_transfered / array_reverse($sub_units)[0]['multiplier']) . '" data-unit="' . explode(' ', array_reverse($sub_units)[0]['name'])[0] . '" >' . $this->transactionUtil->num_f($total_transfered / array_reverse($sub_units)[0]['multiplier'], false, null, true) . '</span> ' . explode(' ', array_reverse($sub_units)[0]['name'])[0];
                })
                ...
                ->editColumn('total_adjusted', function ($row) {
                    ...
                    return '<span class="total_adjusted" data-orig-value="' . ($total_adjusted / array_reverse($sub_units)[0]['multiplier']) . '" data-unit="' . explode(' ', array_reverse($sub_units)[0]['name'])[0] . '" >' . $this->transactionUtil->num_f($total_adjusted / array_reverse($sub_units)[0]['multiplier'], false, null, true) . '</span> ' . explode(' ', array_reverse($sub_units)[0]['name'])[0];
                })
                ->editColumn('unit_price', function ($row) use ($allowed_selling_price_group) {
                    ...
                    if (auth()->user()->can('access_default_selling_price')) {
                        $html .= $this->transactionUtil->num_f($row->unit_price * array_reverse($sub_units)[0]['multiplier'], true);
                    }
        ...
        $data = $datatable->rawColumns($raw_columns)->make(true);
        ...
        $units = Unit::where('business_id', $business_id)
            ->pluck('short_name', 'id');
        $business_locations = BusinessLocation::forDropdown($business_id, true);
        $detailled_business_locations = BusinessLocation::All();

        return view('manuel_report.index')
            ->with(compact('categories', 'brands', 'units', 'business_locations', 'detailled_business_locations', 'show_manufacturing_data', 'data'));
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function getManuelreport2(Request $request)
    {
        if (!auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $query = TransactionSellLine::join(
            ...
            ->join('transaction_payments as tp1', 'transaction_sell_lines.transaction_id', '=', 'tp1.transaction_id') //custom
            ->join('business_locations as bl', 't.location_id', '=', 'bl.id') //custom
            ...
            $query->where('VLD.location_id', '=','bl.id');
            ...
            */
            ...
            //->leftjoin('business_locations as l', 'vld.location_id', '=', 'l.id')//custom
            ->leftjoin('variation_location_details as VLD', 'v.id', '=', 'VLD.variation_id')
            ->where('t.business_id', $business_id)
            ->where('VLD.location_id', DB::raw('bl.id'))
            ->where('t.type', 'sell')
            ->where('t.status', 'final')
            ->select(
                //DB::raw("SUM(IF(t.type = 'sell', final_total, 0)) as _total_invoice"),
                //DB::raw("SUM(SELECT SUM(IF(tp.is_return = 1,-1*tp.amount,tp.amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id) as _invoice_paid"),
                //DB::raw('bl.id as location_id'), //custom
                //DB::raw("count(transaction_sell_lines.id) as total_article2 WHERE transaction_id=381"),
                //DB::raw('SUM(tp.amount) as transact_payments_sold where tp.method=\'custom_pay_1\'' ),
                //DB::raw("SUM((SELECT COALESCE(SUM(tp.amount), 0) FROM transaction_payments as mytp WHERE mytp.transaction_id=150)) as mytotal_paid"),
                //DB::raw('SUM(t.final_total - (SELECT COALESCE(SUM(IF(tp.is_return = 1, -1*tp.amount, tp.amount)), 0) FROM transaction_payments as tp WHERE tp.transaction_id = t.id) )  as invoice_total_due'),
                DB::raw('(t.final_total - (SELECT COALESCE(SUM(IF(tp.is_return = 1, -1*tp.amount, tp.amount)), 0) FROM transaction_payments as tp WHERE tp.transaction_id = t.id) )  as invoice_total_due'),
                'v.id as variable_id',
                'p.id as product_id',
                ...
                't.invoice_no',
                't.discount_amount as discount_amount_by_invoice',
                't.final_total as invoice_final_total',
                't.final_total as invoice_total_before_tax',
                't.transaction_date as transaction_date',

                //DB::raw('count(transaction_sell_lines.id) as total_article where transaction_id=381'),//custom total article
                ...
                //custom
                'VLD.qty_available as stock', //custom
        ...
        if (empty($start_date) && empty($end_date)) {
            ...
            $end_date = \Carbon::yesterday()->addHours(23);
        }
        if (!empty($start_date) && !empty($end_date)) {
            //dd(11111);
        ...
        /*********custom */
        $show_by_box = request()->input('show_by_box');
        ($show_by_box == '1') ? $show_by_box = 'true' : '';
        //dd($show_by_box);
        /****************endcustom */
        $product = null;
        $sub_units = null;
        ...
        $data = Datatables::of($query)
            ->addColumn('stock_history_available', function ($row) use ($business_id, $product, $sub_units, $show_by_box) {
                $id = $row->product_id;
                $id = $row->variable_id;
                $variation = '';
                $stock_details = $this->productUtil->getVariationStockDetails($business_id, $id, $row->location_id);
                $stock_history = $this->productUtil->getVariationStockHistory($business_id, $id, $row->location_id);
                ...
                foreach ($stock_history as $key => $value) {
                    if ($value['date'] == $row->transaction_date) {
                        //dd($v['stock']);
                        $stock += $value['stock'];
                    }
                }

                if (isset($show_by_box) && $show_by_box == 'true') {
                    $product = Product::where('id', $row->product_id)
                        ->with(['unit', 'second_unit'])
                        ->first();
                    //dd($product);
                    ...
                    $stock = (float) ($stock / array_reverse($sub_units)[0]['multiplier']);
                }
                //dd($stock);
                //return '<span class="current_stock" data-orig-value="' . (float) ($stock / array_reverse($sub_units)[0]['multiplier']) . '" data-unit="' . explode(' ', array_reverse($sub_units)[0]['name'])[0] . '"> ' . $this->transactionUtil->num_f(($stock / array_reverse($sub_units)[0]['multiplier']), false, null, true) . '</span>' . ' ' . explode(' ', array_reverse($sub_units)[0]['name'])[0];

                //dd($stock);
                return number_format($stock, 7, '.', '');
                foreach ($stock_history as $key => $value) {
                    $needle = $row->transaction_date;
                    $result = array_filter(
                        $stock_history,
                        function ($v) use ($needle, $stock) {
                ...
                return 2;
                echo $row->product_name . " ===> " . $row->location_name . " ===> " . $row->location_id . "==>date:" . $row->transaction_date . "<==" . $stock;
                /*
                //dd($stock_details);
                dd($stock_history);
                dd('====>' . $row->location_id);
                $stock_history = $this->productUtil->getVariationStockHistory($business_id, $row->product_id, request()->input('location_id'));
                dd($stock_history);
                return $variation;
                */
            })

            ->editColumn('unit', function ($row) use ($business_id, $product, $sub_units, $show_by_box) {
                if (isset($show_by_box) && $show_by_box == 'true') {
                    $product = Product::where('id', $row->product_id)
                        ->with(['unit', 'second_unit'])
                        ->first();
                    ...
                    return explode(' ', array_reverse($sub_units)[0]['name'])[0];
                } else {
                    return $row->unit;
                }
            })
            ->editColumn('unit_allow_decimal', function ($row) use ($business_id, $product, $sub_units, $show_by_box) {
                if (isset($show_by_box) && $show_by_box == 'true') {
                    $product = Product::where('id', $row->product_id)
                        ->with(['unit', 'second_unit'])
                        ->first();
                    ...
                    return explode(' ', array_reverse($sub_units)[0]['allow_decimal'])[0];
                } else {
                    return $row->unit_allow_decimal;
                }
            })
            ...
            ->editColumn('invoice_no', function ($row) {
                return '<a data-href="' . action([\App\Http\Controllers\SellController::class, 'show'], [$row->transaction->id])
            ...
            ->editColumn('sell_qty', function ($row) use ($business_id, $product, $sub_units, $show_by_box) {
                ...
                if (isset($show_by_box) && $show_by_box == 'true') {
                    ...
                    $sell_qty = (float) ($sell_qty / array_reverse($sub_units)[0]['multiplier']);
                }
                return number_format($sell_qty, 7, '.', '');
            })
            ->editColumn('subtotal', function ($row) use ($business_id, $product, $sub_units, $show_by_box) {
                //ignore child sell line of combo product
                $class = is_null($row->parent_sell_line_id) ? 'row_subtotal' : '';
                return number_format($row->subtotal, 7, '.', '');
            })
            ->editColumn('unit_price', function ($row) use ($business_id, $product, $sub_units, $show_by_box) {
                //ignore child sell line of combo product
                ...
                if (isset($show_by_box) && $show_by_box == 'true') {
                    ...
                    $unit_price = (float) ($unit_price / array_reverse($sub_units)[0]['multiplier']);
                }
                return number_format($unit_price, 7, '.', '');
            })
        ...
        /******personnalize custom code 05022024 */
        //$detailled_business_locations = BusinessLocation::All(); //erreur parec que ca liste meme les depots des autres entreprises
        $detailled_business_locations=BusinessLocation::All()->where('business_id', $business_id);
        //$detailled_business_locations = BusinessLocation::All();
        /***************************************** */
        
        //return view('report.product_sell_report')
        return view('manuel_report2.index')
            ->with(
                compact(
                    'business_locations',
                    'customers',
     ...
     * @return \Illuminate\Http\Response
     */
    public function getManuelreport3(Request $request)
    {
        if (!auth()->user()->can('stock_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        /****************endcustom */
        $products = $this->productUtil->getProductStockDetails($business_id, $filters, $for);
        //$data = Datatables::of($products)->make(true);
        //dd($data);
        
        $datatable = Datatables::of($products)
            ->addColumn('stock_history_available', function ($row) use ($business_id, $show_by_box) {

                $stock = $row->stock;


                /**********CUSTOM:SYNCRONIZE STOCK HISTORY TO AVOID MISTAKE****** */
                $id = $row->variation_id;
            ...
            ->editColumn('unit', function ($row) use ($business_id, $show_by_box) {
                if (isset($show_by_box) && $show_by_box == 'true') {
                    $product = Product::where('id', $row->product_id)
                        ->with(['unit', 'second_unit'])
                        ->first();
            ...
            ->editColumn('unit_allow_decimal', function ($row)  use ($business_id, $show_by_box) {
                if (isset($show_by_box) && $show_by_box == 'true') {
                    $product = Product::where('id', $row->product_id)
                        ->with(['unit', 'second_unit'])
                        ->first();
            ...
            ->editColumn('sell_qty', function ($row)  use ($business_id, $show_by_box) {
                //ignore child sell line of combo product
                $class = is_null($row->parent_sell_line_id) ? 'sell_qty' : '';
                $sell_qty = $row->sell_qty;
                if (isset($show_by_box) && $show_by_box == 'true') {
            ...
            ->editColumn('subtotal', function ($row) use ($business_id, $show_by_box) {
                //ignore child sell line of combo product
                $class = is_null($row->parent_sell_line_id) ? 'row_subtotal' : '';
                return number_format($row->subtotal, 7, '.', '');
            })
            ->editColumn('unit_price', function ($row) use ($business_id, $show_by_box) {
                //ignore child sell line of combo product
                $unit_price = $row->unit_price;
                if (isset($show_by_box) && $show_by_box == 'true') {
                    $product = Product::where('id', $row->product_id)
            ...
            ->addColumn('product_name', function ($row) use ($business_id, $show_by_box) {
                return $row->product;
            })


            ->addColumn('sell_qty', function ($row) use ($business_id, $show_by_box) {
                return 1;
            })
            ->addColumn('subtotal', function ($row) use ($business_id, $show_by_box) {
                return 5;
            })



            ->addColumn('invoice_no', function ($row) use ($business_id, $show_by_box) {
                return 0;
            })

            ->addColumn('discount_amount_by_invoice', function ($row) use ($business_id, $show_by_box) {
                return 0;
            })
            ->addColumn('invoice_final_total', function ($row) use ($business_id, $show_by_box) {
                return 0;
            })
            ->addColumn('invoice_total_before_tax', function ($row) use ($business_id, $show_by_box) {
                return 0;
            })
            ->addColumn('transaction_date', function ($row) use ($business_id, $show_by_box) {
                return 0;
            })
            ->addColumn('transaction_id', function ($row) use ($business_id, $show_by_box) {
                return 0;
            });
        $raw_columns = [
            'unit_price',
        ...
        return view('manuel_report3.index')
            ->with(compact('categories', 'brands', 'units', 'business_locations', 'detailled_business_locations', 'show_manufacturing_data', 'customers', 'customer_group', 'data'));
    }
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            //dd($filters);
            if (isset($filters['show_by_box']) && (Boolean)$filters['show_by_box'] == 'true') {
                $datatable = Datatables::of($products)
                    ->editColumn('stock', function ($row) {
                        $product = Product::where('id', $row->product_id)
                            ->with(['unit', 'second_unit', 'box_unit'])
                            ->first();
                        $business_id = request()->session()->get('user.business_id');
                        $sub_units = $this->productUtil->getSubUnits($business_id, $product->unit->id, false, $row->product_id);

                        $box_unit= empty($product->box_unit) ? $product->unit : $product->box_unit ;
                        foreach ($sub_units as $sub_unit_key => $sub_unit) {
                            if($sub_unit_key==$box_unit->id){
                                $sub_units=[];$sub_units[$sub_unit_key]=$sub_unit;
                                break;
                            }
                        }
    ...
    public function getSalesRepresentativeTotalCommission(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if ($commsn_calculation_type == 'payment_received') {
                ...
                /*******  personnalize custom code 09092024-GET_TOTAL_OF_EXPENSE-001  */
                $filters = $request->only(['expense_for', 'location_id', 'start_date', 'end_date']);
                $total_expense = $this->transactionUtil->getExpenseReport($business_id, $filters, 'total');
                /*********end custom */
                return [
    ...
    public function getproductPurchaseReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $variation_id = $request->get('variation_id', null);
             ...
             /*********custom */
             $show_by_box = (Boolean)request()->input('show_by_box');
             /****************endcustom */
            ...
            return Datatables::of($query)
                ...
                ->editColumn('purchase_qty', function ($row) use ($show_by_box) {
                    $html = '<span data-is_quantity="true" class="display_currency purchase_qty" data-currency_symbol=false data-orig-value="' . (float) $row->purchase_qty . '" data-unit="' . $row->unit . '" >' . (float) $row->purchase_qty . '</span> ' . $row->unit;
                    //dd($row);
                    if (isset($show_by_box) && (Boolean)$show_by_box == 'true') {
                        $product = Product::where('id', $row->product_id)
                            ->with(['unit', 'second_unit', 'box_unit'])
                            ->first();
                        $business_id = request()->session()->get('user.business_id');
                        ...
                        $html = '<span data-is_quantity="true" class="display_currency purchase_qty" data-currency_symbol=false data-orig-value="' . (float) ($row->purchase_qty / array_reverse($sub_units)[0]['multiplier']) . '" data-unit="' . explode(' ', array_reverse($sub_units)[0]['name'])[0] . '" >' . (float) ($row->purchase_qty / array_reverse($sub_units)[0]['multiplier']) . '</span> ' . explode(' ', array_reverse($sub_units)[0]['name'])[0];
                    }
                    return $html;
                
                })
                ->editColumn('quantity_adjusted', function ($row)  use ($show_by_box){
                    $html = '<span data-is_quantity="true" class="display_currency quantity_adjusted" data-currency_symbol=false data-orig-value="' . (float) $row->quantity_adjusted . '" data-unit="' . $row->unit . '" >' . (float) $row->quantity_adjusted . '</span> ' . $row->unit;
                    ...
                    if (isset($show_by_box) && $show_by_box == 'true') {
                        ...
                        $html = '<span data-is_quantity="true" class="display_currency quantity_adjusted" data-currency_symbol=false data-orig-value="' . (float) ($row->quantity_adjusted / array_reverse($sub_units)[0]['multiplier']) . '" data-unit="' . explode(' ', array_reverse($sub_units)[0]['name'])[0] . '" >' . (float) ($row->quantity_adjusted / array_reverse($sub_units)[0]['multiplier']) . '</span> ' . explode(' ', array_reverse($sub_units)[0]['name'])[0];
                    }
                    return $html;
                ...
                ->editColumn('unit_purchase_price', function ($row)  use ($show_by_box) {
                    ...
                    if (isset($show_by_box) && $show_by_box == 'true') {
                        ...
                        return $this->transactionUtil->num_f($row->unit_purchase_price * array_reverse($sub_units)[0]['multiplier'],true);
                    }
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    'transaction_sell_lines.unit_price_before_discount as unit_price',
                    'transaction_sell_lines.unit_price_inc_tax as unit_sale_price',
                    /*******  personnalize custom code 26042024-MANAGE_CUSTOM_PRODUCT_PRICE-001  */
                    'transaction_sell_lines.unit_price_before_customupdate as unit_price_before_customupdate',
                    'transaction_sell_lines.line_discount_amount as line_discount_amount',
                    /*********end custom */
                    /*******  personnalize custom code 09092024-ADD_PAYMENT_INFO_OF_TRANSACTION  */
                    't.status',
                    DB::raw('COUNT( DISTINCT transaction_sell_lines.id) as total_items'),
            ...
            return Datatables::of($query)
                ...
                ->editColumn('total_paid', function ($row) {
                    $class =  'row_total_paid';

                    return '<span class="' . $class . '"  data-orig-value="' . $row->total_paid . '">' .
                        $this->transactionUtil->num_f($row->total_paid, true) . '</span>';
                })
                ->editColumn('total_no_paid', function ($row) {
                    $class =  'row_total_no_paid';
                    $total_no_paid=$row->subtotal-$row->total_paid;
                    return '<span class="' . $class . '"  data-orig-value="' . $total_no_paid . '">' .
                        $this->transactionUtil->num_f($total_no_paid, true) . '</span>';
                })
                /*********end custom */
                /*******  personnalize custom code 26042024-MANAGE_CUSTOM_PRODUCT_PRICE-001  */
                ->editColumn('unit_price_diff', function ($row) {
                    //$unit_price_diff=$row->line_discount_amount;
                    $unit_price_diff=($row->unit_price-$row->unit_price_before_customupdate-$row->line_discount_amount)*$row->sell_qty;
                    //$unit_price_diff=$row->unit_price_before_customupdate*$row->sell_qty;
                    //$unit_price_diff=$row->unit_price_before_customupdate;
                    $sign='';
                    //$unit_price_diff=$row->unit_price_before_customupdate;
                    ...
                    /*
                    return '<span class="unit_price_diff"  data-orig-value="' . $this->transactionUtil->num_f($row->subtotal, true) . '">' .
                        $this->transactionUtil->num_f($row->subtotal, true) . '</span>';
                        */
                    return $sign.$this->transactionUtil->num_f($unit_price_diff, true) ;
                })
                /*********end custom */
                ->editColumn('discount_amount', '
                    @if($discount_type == "percentage")
    ...
    public function getStockValue()
    {
        ...
        $location_id = request()->input('location_id');
        $filters = request()->only(['category_id', 'sub_category_id', 'brand_id', 'unit_id','show_by_box']);
        //dd($filters);
        //Get Closing stock
