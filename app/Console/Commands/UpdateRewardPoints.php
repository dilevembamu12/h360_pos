...
class UpdateRewardPoints extends Command
{
    ...
    protected $signature = 'pos:updateRewardPoints {--reset}';
    ...
    public function handle()
    {
        ...
        try {
            ...
            foreach ($businesses as $business) {
                ...
                //prendre en compte le reset, si c'est unreset on rallonge la date d'expiration pour que toutes les transaction y figures
                if($this->option('reset')){
                    $transaction_date_to_be_expired->addYears(1000);
                }
                $transactions = Transaction::where('business_id', $business->id)
                                        ...
                                        ->where('rp_reset', false)
                                        ->with(['contact'])
                                        ->select(
                                            DB::raw('SUM(COALESCE(rp_earned, 0)) as total_rp_expired'),
                                            'contact_id','id as transaction_id'
                                        )
                                        ->groupBy('contact_id')
                                        ->get();
                ...
                foreach ($transactions as $key => $transaction) {
                    ...
                    if ((! empty($transaction->total_rp_expired) && $transaction->contact->total_rp_used < $transaction->total_rp_expired) 
                        ...
                        //je gere le cas ou le bonus est negatif je ramene à zero
                        $contact->total_rp =($contact->total_rp <0) ? 0 : $contact->total_rp ;
                        //dd($contact->id);
                        $contact->total_rp_expired = $transaction->total_rp_expired;
                        $contact->save();
                        //signale les bonus de cette transaction comme étant réinitalisés
                        /*
                        DB::table('contacts')->where('id',$contact->id)
                                        ->update(['total_rp'=>$contact->total_rp]);
                                        */
                        DB::table('transactions')->where('id',$transaction->transaction_id)
                                        ->update(['rp_reset'=>!$this->option('reset')]);
                    }
                }
            }
