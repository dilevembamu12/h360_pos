...
// ========================================================================
// SECTION 1 : COMPILATION DES ASSETS PRINCIPAUX DE L'APPLICATION
// ========================================================================
...
// ========================================================================
// SECTION 2 : COMPILATION DES MODULES
// ========================================================================

// Liste des modules à ne PAS compiler.
const modulesToSkip = [
    ...
    "AiAssistance", "InventoryManagement", "Hms", "Hopital", "Whatsapp", "Hospital",
];

// Fusionne le manifeste principal avec ceux des modules
mix.mergeManifest();

// Cherche et compile les assets de chaque module
const modulesPath = 'Modules';
const directories = fs.readdirSync(modulesPath)
    .filter(file => {
        const isDirectory = fs.statSync(path.join(modulesPath, file)).isDirectory();
        const shouldSkip = modulesToSkip.includes(file);
        return isDirectory && !shouldSkip;
    });

directories.forEach(module => {
    const mixFilePath = `${modulesPath}/${module}/webpack.mix.js`;
    if (fs.existsSync(mixFilePath)) {
        require('./' + mixFilePath);
    }
});

// Ajout de la compilation de pos.js qui n'était pas dans la première partie
