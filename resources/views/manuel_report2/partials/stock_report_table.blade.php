...
@php
    ...
    $formatted_array = [];
    //$formatted_array = ['PRODUCT 1'=>['product_name' => 'PRODUCT 1', 'stock retour' => 12, 'stock vendu' => 23, 'solde' => 23]];
    
    foreach ($data->original['data'] as $key => $item) {
        $amount_net_by_invoice[$item['location_name'] . '::' . $item['location_id']][$item['transaction_id']] = [
            'invoice_final_total' => $item['invoice_final_total'],
            'invoice_total_before_tax' => $item['invoice_total_before_tax'],
            'invoice_total_due' => $item['invoice_total_due'],
            'discount_amount_by_invoice' => $item['discount_amount_by_invoice'],
    ...
    foreach ($detailled_business_locations as $key => $value) {
        ...
        if ($value->custom_field1 == 1) { //gestion ancien code (2023)
            ...
            if($value->is_deposit == 1){ //gestion nouveau code (05022024)
                //dd($value->canopy_ids);
...
@php
    ...
    $product_custom_field1 = !empty($custom_labels['product_name']['custom_field_1']) ? $custom_labels['product_name']['custom_field_1'] : __('lang_v1.product_custom_field1');
    $product_custom_field2 = !empty($custom_labels['product_name']['custom_field_2']) ? $custom_labels['product_name']['custom_field_2'] : __('lang_v1.product_custom_field2');
    $product_custom_field3 = !empty($custom_labels['product_name']['custom_field_3']) ? $custom_labels['product_name']['custom_field_3'] : __('lang_v1.product_custom_field3');
    $product_custom_field4 = !empty($custom_labels['product_name']['custom_field_4']) ? $custom_labels['product_name']['custom_field_4'] : __('lang_v1.product_custom_field4');
@endphp
...
@endphp
@foreach ($deposit_with_canopy as $key => $item)
    @component('components.widget', ['title' => $key, 'class' => 'box-primary', 'id' => $key])
    ...
    @php
        if (count($canopy_money_array_title_for_datatable) > 2) {
            $whole_titles2 = $canopy_money_array_title_for_datatable;
        }
        //dd($resume_all_by_money);
    @endphp
@endforeach
    ...
    <div class="col-md-12">
        ...
        @component('components.widget', ['title' => 'Tous les rapports'])
            ...
            <div class="row">
                ...
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <h1>CASH VENTE GLOBAL (Dollars ($)).</h1>
        ...
        $(document).ready(function() {
            @foreach ($deposit_with_canopy as $key => $item)
                @php
                    ...
                    $titles = $resume_all_by_sell_qty[$key . '::' . $item['id']]['title'];
                @endphp
                @if (count($resume_all_by_sell_qty[$key . '::' . $item['id']]['data']) > 0)
                    $('#tableID-{{ $item['id'] }}').DataTable({
                        ...
                        data: @json($resume_all_by_sell_qty[$key . '::' . $item['id']]['data']),
                ...
                @if (count($resume_all_by_money[$key . '::' . $item['id']]['data']) > 0)
            ...
            console.log('tototot', @json($whole_titles2));
