...
@php
  //dd($all_units);
  //dd($product->product_locations->pluck('id'));
    ...
    @component('components.widget', ['class' => 'box-primary'])
        <div class="row">
            ...
            <div class="col-sm-4 @if(!session('business.enable_sub_units')) hide @endif">
              ...
              {{--- custom :mettre les box_unit comme unité de gros----------}}
              <div class="form-group">
                {!! Form::label('box_unit_id', '** Unité de gros' . ':') !!} @show_tooltip(__('lang_v1.sub_units_tooltip'))

                <select name="box_unit_id" class="form-control select2"  id="box_unit_id">
                  @foreach($all_units as $all_unit_id => $all_unit_value)
                    <option value="{{$all_unit_id}}" 
                      @if( $all_unit_id == $product->box_unit_id)   selected 
                      @endif>{{$all_unit_value}}</option>
                  @endforeach
                </select>
              </div>
              {{---------------------}}
