    ...
    @component('components.filters', ['title' => __('report.filters')])
        ...
        <div class="col-md-3">
            ...
            <div class="form-group">
                <br>
                <label>
                    {!! Form::checkbox('active_canopy', 1, false, ['class' => 'input-icheck', 'id' => 'active_canopy']) !!} <strong>Activer le canopy</strong>
                </label>
            </div>
            --}}
        </div>
...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready( function(){
            product_table = $('#product_table').DataTable({
                ...
                "ajax": {
                    ...
                    "data": function ( d ) {
                        ...
                        d.show_by_box = $('#show_by_box').is(':checked'); //custom
            ...
            /******* custom code ************/
            $(document).on('ifChanged', '#show_by_box', function() {
                if ($("#product_list_tab").hasClass('active')) {
                    product_table.ajax.reload();
                }
        ...
        $(document).on('shown.bs.modal', 'div.view_product_modal, div.view_modal, #view_product_modal', 
            ...
            if (div.length) {
                $.ajax({
                    url: "{{action([\App\Http\Controllers\ReportController::class, 'getStockReport'])}}"  + '?for=view_product&product_id=' + div.data('product_id') + "&show_by_box=" + $(
                                '#show_by_box').is(':checked'),//custom 
                    dataType: 'html',
                    success: function(result) {
                        div.html(result);
