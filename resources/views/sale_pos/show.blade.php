@php
    ...
    $currencies = Currency::find([$business->currency_id,$sell->second_currency]);
        ...
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="modalTitle"> @lang('sale.sell_details') (<b>
                ...
                @if (!empty($sell->second_currency) && session()->all()['business']->currency_id != $sell->second_currency)
                    <a class="btn btn-info btn-xs default_currency" href="#!" onclick="$('.default_currency').hide();$('.other_currency').show();"><i
                            class="fas fa-money-bill-alt fa-lg"></i> ***Autre devise </a>
                    <a class="btn btn-info btn-xs other_currency" href="#!"  onclick="$('.default_currency').show();$('.other_currency').hide();"><i
                              class="fas fa-money-bill-alt fa-lg"></i> ***Devise par defaut </a>
                @endif
        ...
        <div class="modal-body">
            ...
            <div class="row">
                ...
                <div class="@if (!empty($export_custom_fields)) col-sm-3 @else col-sm-4 @endif">
                  <b class="default_currency" style="font-size: 2em; color:red">{{$currencies[0]->currency}}({{$currencies[0]->code}})</b>
                  @if (!empty($sell->second_currency) && session()->all()['business']->currency_id != $sell->second_currency)
                  <b class="other_currency" style="color:red"><span style="font-size: 2em; ">{{$currencies[1]->currency}}</span><br>
                    <span>(1{{$currencies[0]->code}} = {{number_format($sell->second_currency_rate,4)}}{{$currencies[1]->code}})</b></span>
                  @endif
                  <br>  
                  <b>
                        @if ($sell->type == 'sales_order')
            ...
            <div class="row default_currency">
                @php
                    $total_paid = 0;
                @endphp
                @if ($sell->type != 'sales_order')
                                ...
                                @foreach ($sell->payment_lines as $payment_line)
                                    @php
                                        if ($payment_line->is_return == 1) {
                                            $total_paid -= $payment_line->getOriginal('amount');
                                        } else {
                                            $total_paid += $payment_line->getOriginal('amount');
                                        }
                                    ...
                                    <tr>
                                        ...
                                        <td><span class="display_currency"
                                                data-currency_symbol="true">{{ $payment_line->getOriginal('amount') }}</span></td>
                                        <td>
            ...
            @if (!empty($sell->second_currency) && session()->all()['business']->currency_id != $sell->second_currency)
                ...
                <div class="row other_currency">
                        ...
                        <div class="col-md-6 col-sm-12 col-xs-12">
                            <div class="table-responsive">
                                <table class="table bg-gray">
                                    ...
                                    @foreach ($sell2->payment_lines as $payment_line)
                                        @php
                                            if ($payment_line->is_return == 1) {
                                                $total_paid -= $payment_line->getOriginal('amount') * $sell2->second_currency_rate;
                                            } else {
                                                $total_paid += $payment_line->getOriginal('amount')* $sell2->second_currency_rate;
                                            }
                                        @endphp
                                        <tr>
                                            ...
                                            <td>{{ $payment_line->payment_ref_no }}</td>
                                            <td><span class="display_currency"
                                                    data-currency_symbol="true">{{ $payment_line->getOriginal('amount')* $sell2->second_currency_rate }}</span>
                                            </td>
                                            <td>
                                                {{ $payment_types[$payment_line->method] ?? $payment_line->method }}
                                                @if ($payment_line->is_return == 1)
                    ...
                    @endif
                    <div class="col-md-6 col-sm-12 col-xs-12 @if ($sell2->type == 'sales_order') col-md-offset-6 @endif">
                        <div class="table-responsive">
                            <table class="table bg-gray">
                                <tr>
                                    <th>{{ __('sale.total') }}: </th>
                                    <td></td>
                                    <td><span class="display_currency pull-right"
                                            data-currency_symbol="true">{{ $sell2->total_before_tax }}</span></td>
                                </tr>
                                <tr>
                                    <th>{{ __('sale.discount') }}:</th>
                                    <td><b>(-)</b></td>
                                    <td>
                                        <div class="pull-right"><span class="display_currency"
                                                @if ($sell2->discount_type == 'fixed') data-currency_symbol="true" @endif>{{ $sell2->discount_amount }}</span>
                                            @if ($sell2->discount_type == 'percentage')
                                                {{ '%' }}
                                            @endif
                                            </span>
                                        </div>
                                ...
                                @if (in_array('types_of_service', $enabled_modules) && !empty($sell->packing_charge))
                                    <tr>
                                        ...
                                        <td>
                                            <div class="pull-right"><span class="display_currency"
                                                    @if ($sell2->packing_charge_type == 'fixed') data-currency_symbol="true" @endif>{{ $sell2->packing_charge }}</span>
                                                @if ($sell2->packing_charge_type == 'percent')
                                                    {{ '%' }}
                                                @endif
                                            </div>
                                        </td>
                                ...
                                @endif
                                @if (session('business.enable_rp') == 1 && !empty($sell2->rp_redeemed))
                                    <tr>
                                        <th>{{ session('business.rp_name') }}:</th>
                                        <td><b>(-)</b></td>
                                        <td> <span class="display_currency pull-right"
                                                data-currency_symbol="true">{{ $sell2->rp_redeemed_amount }}</span>
                                        </td>
                                    </tr>
                                @endif
                                <tr>
                                ...
                                @endif
                                <tr>
                                    <th>{{ __('sale.shipping') }}: @if ($sell2->shipping_details)
                                            ({{ $sell2->shipping_details }})
                                        @endif
                                    </th>
                                    <td><b>(+)</b></td>
                                    <td><span class="display_currency pull-right"
                                            data-currency_symbol="true">{{ $sell2->shipping_charges }}</span></td>
                                </tr>

                                @if (!empty($sell2->additional_expense_value_1) && !empty($sell2->additional_expense_key_1))
                                    <tr>
                                        <th>{{ $sell2->additional_expense_key_1 }}:</th>
                                        <td><b>(+)</b></td>
                                        <td><span
                                                class="display_currency pull-right">{{ $sell2->additional_expense_value_1 }}</span>
                                        </td>
                                    </tr>
                                @endif
                                @if (!empty($sell2->additional_expense_value_2) && !empty($sell2->additional_expense_key_2))
                                    <tr>
                                        <th>{{ $sell2->additional_expense_key_2 }}:</th>
                                        <td><b>(+)</b></td>
                                        <td><span
                                                class="display_currency pull-right">{{ $sell2->additional_expense_value_2 }}</span>
                                        </td>
                                    </tr>
                                @endif
                                @if (!empty($sell2->additional_expense_value_3) && !empty($sell2->additional_expense_key_3))
                                    <tr>
                                        <th>{{ $sell2->additional_expense_key_3 }}:</th>
                                        <td><b>(+)</b></td>
                                        <td><span
                                                class="display_currency pull-right">{{ $sell2->additional_expense_value_3 }}</span>
                                        </td>
                                    </tr>
                                @endif
                                @if (!empty($sell2->additional_expense_value_4) && !empty($sell2->additional_expense_key_4))
                                    <tr>
                                        <th>{{ $sell2->additional_expense_key_4 }}:</th>
                                        <td><b>(+)</b></td>
                                        <td><span
                                                class="display_currency pull-right">{{ $sell2->additional_expense_value_4 }}</span>
                                        </td>
                                    </tr>
                                @endif
                                <tr>
                                    ...
                                    <td></td>
                                    <td><span class="display_currency pull-right"
                                            data-currency_symbol="true">{{ $sell2->round_off_amount }}</span></td>
                                </tr>
                                <tr>
                                    <th>{{ __('sale.total_payable') }}: </th>
                                    <td></td>
                                    <td><span class="display_currency pull-right"
                                            data-currency_symbol="true">{{ $sell2->final_total }}</span></td>
                                </tr>
                                @if ($sell2->type != 'sales_order')
                                    <tr>
                                        <th>{{ __('sale.total_paid') }}:</th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @endphp
                                            <span class="display_currency pull-right"
                                                data-currency_symbol="true">{{ $sell2->final_total - $total_paid }}</span>
                                        </td>
                                    </tr>
                                @endif
                            </table>
