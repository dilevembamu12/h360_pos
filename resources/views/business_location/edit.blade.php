{{-- -- custom pour le select2 multiple fonctionne avec le canopy_ids --
...
@php
    //dd($location->second_currency);
    //$canopy_ids=collect($location->canopy_ids);
    //dd($canopy_ids);
    //dd( $location->canopy_ids);
@endphp
            ...
            <div class="row">
                ...
                <div class="col-sm-4">
                    <div class="form-group">
                        <br>
                        <label>
                            {!! Form::checkbox('is_deposit', 1, !empty($duplicate_product) ? $duplicate_product->enable_stock : true, [
                                'class' => 'input-icheck',
                                'id' => 'is_deposit',
                            ]) !!} <strong>**Est un depot</strong>
                ...
                <div class="col-sm-8">
                    <div class="form-group">
                        {!! Form::label('canopy_ids', 'Les canopys' . ':') !!} @show_tooltip(__('lang_v1.sub_units_tooltip'))

                        {!! Form::select('canopy_ids[]', $business_locations, $location->canopy_ids, [
                ...
                {{--  personnalize custom code 15032024-MULTIDEVISE002 --}}
                <div class="clearfix"></div>
                <hr>
                <div class="col-sm-12">
                    <div class="form-group">
                        {!! Form::label('second_currency', "**taux d'echange d'échange local" . ':') !!} @show_tooltip("le taux d'échange au niveau local où se trouve ce point de vente  ") 
                ...
                @php
                    ...
                    $businesslocation_currencies = [];
                    foreach ($__currencies as $key => $value) {
                        //dd(str_replace('currency_','',$key));
                        $businesslocation_currencies[str_replace('currency_', '', $key)] =
                            $currencies[str_replace('currency_', '', $key)] . "[$value]";
                    }
                    $location->second_currency = !empty($location->second_currency) ? $location->second_currency : 0;
                @endphp

                <div class="col-sm-6">
                    <div class="form-group">
                        {!! Form::text('second_currency_rate', @num_format($location->second_currency_rate), [
                            'class' => 'form-control',
                            'placeholder' => "**taux d'echange",
                        ]) !!}
                    </div>
                </div>
                <div class="col-sm-6">
                    <div class="form-group">
                        {!! Form::select('second_currency', $businesslocation_currencies, $location->second_currency, [
                            'class' => 'form-control',
                            'placeholder' => __('messages.please_select'),
                        ]) !!}
                    </div>
