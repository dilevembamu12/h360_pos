...
@php
	...
	$second_currency = Currency::find($business->currency_id);
	if(!empty($_GET['currency'])){
		//je verifie si il existe comme devise
		$currency_id=$_GET['currency'];
		if(!empty($_currencies[$_GET['currency']])){
			$currency_id=$business->currency_id;
		}
		$second_currency = Currency::find($_GET['currency']);
...
<section class="content">
	...
	@if(empty($_GET['currency']))
		...
		<input type="hidden" id="currency_id" value="{{$business->currency_id}}">
	@else
		@if( $_GET['currency']!=str_replace('currency_','',$key)  && $_GET['currency']!=$business->currency_id )
		...
		<!-- Page level currency setting -->
		<input type="hidden" id="p_code" value="{{$second_currency->code}}">
		<input type="hidden" id="p_symbol" value="{{$second_currency->symbol}}">
		<input type="hidden" id="p_thousand" value="{{$second_currency->thousand_separator}}">
		<input type="hidden" id="p_decimal" value="{{$second_currency->decimal_separator}}">


		<input type="hidden" id="currency_id" value="{{str_replace('currency_','',$key)}}">
	...
	@component('components.widget', ['class' => 'box-primary'])
		<div class="row">
			...
			{{--
			<div class="col-sm-12">
				<h4 class="text-center">***Autres devises:</h4>
				<p class="help-block text-center">
					<b>Vous pouvez faire vos achats avec une devise secondaire</b>
				</p>
			...
			@foreach ($currencies as $key => $value)
				@if(empty($_GET['currency']))
					@if(str_replace('currency_','',$key)!=$business->currency_id )
					<div class="col-sm-3">
						<a class="btn btn-info btn-xs" href="{{action([\App\Http\Controllers\PurchaseController::class, 'create'])}}?currency={{str_replace('currency_','',$key) }}"><i class="fas fa-history"></i> {{$value}}</a>
					</div>
					@endif
				@else
					@if( $_GET['currency']!=str_replace('currency_','',$key) /*  && $_GET['currency']!=$business->currency_id */)
	...
	@if(!empty($_GET['currency']))
		<input type="hidden" name="currency_id" value="{{str_replace('currency_','',$key)}}">
	@endif
	{{--  *********************************************** --}}
