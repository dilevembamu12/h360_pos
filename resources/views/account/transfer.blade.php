<div class="modal-dialog" role="document">
  <div class="modal-content">
    ...
    <div class="modal-body">
            ...
            {{-- custom 002 : le compte ou vient largent ne doit pas etre à choisir : il vient deja avec le compte par defaut --}}
            @php
            $from_accounts=[];
            foreach ($to_accounts as $key => $to_account) {
              if($key==$from_account->id){
                $from_accounts[$key]=$to_account;
              }
            }
            //j'enleve le  compte sender coté compte receiver
            foreach ($to_accounts as $key => $to_account) {
              if($key==$from_account->id){
            ...
            <div class="form-group">
                ...
                {{-- {!! Form::select('from_account', $from_accounts, $from_account->id, ['class' => 'form-control', 'required' ]); !!} --}}
                {!! Form::select('from_account', $from_accounts, $from_account->id, ['class' => 'form-control', 'required' ]); !!}
            </div>
